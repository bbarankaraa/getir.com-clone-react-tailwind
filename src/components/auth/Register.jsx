import ReactFlagsSelect from "react-flags-select";
import { IoIosClose } from "react-icons/io";
import { useState } from "react";

const Register = (props) => {

  const closeClickHandler = () => {
    props.setRegisterOpened(false);
  };
  
  const [selected, setSelected] = useState("TR");
  const phones = { US: "+1", TR: "+90" };

  return (
    <div className="h-[100vh] w-[100vw] bg-black/50 flex justify-center items-center flex-col fixed top-0 right-0 z-50">
      <div className="w-full md:w-[440px] md:h-auto h-full md:rounded-lg bg-white">
        <div className="text-center pt-8 relative">
          <h4 className="text-brand-color font-semibold">
            Kayıt Ol
          </h4>
          <IoIosClose
            size={30}
            className="bg-gray-200 rounded-md absolute top-0 right-0 mx-4 my-8 cursor-pointer"
            onClick={closeClickHandler}
          />
        </div>
        <div className="grid gap-y-3 px-8">
          <div className="flex gap-4 flex-col pt-8">
            <div className="flex gap-x-2">
            <ReactFlagsSelect
              countries={Object.keys(phones)}
              customLabels={phones}
              selected={selected}
              onSelect={(code) => setSelected(code)}
              className="flag-select"
            />
            <label className="flex-1 relative block">
              <input
                required
                type="text"
                className="h-14 px-4 border-2 border-gray-200 rounded w-full transition-colors hover:border-brand-color outline-0 focus:border-brand-color peer text-sm pt-2"
              />
              <span className="absolute top-0 left-0 h-full px-4 flex items-center text-sm text-gray-400 transition-all peer-focus:h-7 peer-focus:text-brand-color peer-focus:text-xs peer-valid:h-7 peer-valid:text-brand-color peer-valid:text-xs">
                Telefon Numarası
              </span>
            </label>
            </div>
            <div className="flex flex-col gap-4">
            <label className="flex-1 relative block">
              <input
                required
                type="text"
                className="h-14 px-4 border-2 border-gray-200 rounded w-full transition-colors hover:border-brand-color outline-0 focus:border-brand-color peer text-sm pt-2"
              />
              <span className="absolute top-0 left-0 h-full px-4 flex items-center text-sm text-gray-400 transition-all peer-focus:h-7 peer-focus:text-brand-color peer-focus:text-xs peer-valid:h-7 peer-valid:text-brand-color peer-valid:text-xs">
                Ad Soyad
              </span>
            </label>
            <label className="flex-1 relative block">
              <input
                required
                type="text"
                className="h-14 px-4 border-2 border-gray-200 rounded w-full transition-colors hover:border-brand-color outline-0 focus:border-brand-color peer text-sm pt-2"
              />
              <span className="absolute top-0 left-0 h-full px-4 flex items-center text-sm text-gray-400 transition-all peer-focus:h-7 peer-focus:text-brand-color peer-focus:text-xs peer-valid:h-7 peer-valid:text-brand-color peer-valid:text-xs">
                E-Posta
              </span>
            </label>
            </div>
          </div>
          <div className="text-xs text-paragraph">
            <p>
              Kişisel verilerine dair Aydınlatma Metni için
              <span>
                <a
                  href="#"
                  className="text-brand-color underline hover:text-primary-color px-1"
                >
                  tıklayınız.
                </a>
              </span>
              Üye olmakla, <span><a href="#" className="text-brand-color underline hover:text-primary-color">Kullanım Koşulları</a></span> hükümlerini kabul etmektesin.
            </p>
          </div>
        </div>
        <div className="flex gap-4 flex-col px-8 mt-10 mb-2">
            <button className="bg-brand-color text-white transition-colors hover:bg-primary-color h-12 flex items-center justify-center w-full rounded-md text-sm font-semibold cursor-pointer">
            Kayıt Ol
          </button>
        </div>
        <div className="bg-gray-100 py-4 md:rounded-b-lg text-paragraph text-sm text-center">
          <p>
            Getir'e üyeysen
            <span>
              <a href="" className="text-brand-color font-semibold p-1">
                Giriş yap
              </a>
            </span>
          </p>
        </div>
      </div>
    </div>
  );
};

export default Register;
